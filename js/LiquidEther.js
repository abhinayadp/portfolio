// Three.js is loaded via global script tag in index.html
...
class LiquidEther {
    constructor(container, options = {}) {
        ...
        this.options = {
            ...
            autoResumeDelay: options.autoResumeDelay ?? 5000,
    ...
    init() {
        const { colors, mouseForce, cursorSize, isViscous, viscous, iterationsViscous, iterationsPoisson, dt, BFECC, resolution, isBounce, autoDemo, autoSpeed, autoIntensity, takeoverDuration, autoResumeDelay, autoRampDuration } = this.options;
...
void main(){
  ...
  // Strictly hide the effect when there is no active movement
  if (lenv < 0.005) {
    discard;
  }
  float normalizedLen = clamp(lenv, 0.0, 1.0);
  vec3 c = texture2D(palette, vec2(normalizedLen, 0.5)).rgb;
  
  // Use normalized length for alpha, with a quick falloff
  float alpha = clamp(lenv * 2.0, 0.0, 0.7);
...
`;
        ...
        class Advection extends ShaderPass {
                ...
                super({
                    material: {
                        ...
                        uniforms: {
                            ...
                            dissipation: { value: simProps.dissipation || 0.96 },
                            isBFECC: { value: true }
                        }
                    },
                    output: simProps.dst
            ...
            update({ dt, isBounce, BFECC, dissipation }) {
                ...
                this.uniforms.dissipation.value = dissipation || 0.96;
                super.update();
            }
        }
        ...
        class Simulation {
            ...
            createShaderPass() {
                this.advection = new Advection({ cellScale: this.cellScale, fboSize: this.fboSize, dt: this.options.dt, dissipation: this.options.dissipation, src: this.fbos.vel_0, dst: this.fbos.vel_1 });
            ...
            calcSize() {
                ...
                const px_x = 1.0 / width; const px_y = 1.0 / height;
            ...
            update() {
                ...
                this.advection.update({ dt: this.options.dt, isBounce: this.options.isBounce, BFECC: this.options.BFECC, dissipation: this.options.dissipation });
        ...
        Object.assign(sim.options, {
            ...
            dt, BFECC, resolution, isBounce, dissipation: this.options.dissipation
        });
        sim.resize();
